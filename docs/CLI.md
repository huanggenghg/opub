...
发布到 B站需要本地 biliup 上传程序。发布与 `--dry-run` 会在首次账号检查之前对启用 B站的任务做只读程序检查，缺失或不可执行时返回 `ENV-007` 并停止，不自动下载；运行 `opub --repair-env --with-bilibili` 显式安装或更新，普通 `--repair-env` 不会安装它。B站子进程按阶段限时：查询 60 秒、扫码登录 360 秒、上传 3600 秒；上传超时视为结果未确认，不可自动重试，按 `RUN-004` 语义人工核对后再创建新任务。

B站任务取消后会终止并回收当前子进程，不再执行后续命令；如果投稿已经开始，取消不能撤回平台已收到的内容，仍需人工核对结果。显式修复会恢复已有 biliup 文件的执行权限，并在报告完成前重新检查程序是否可执行。
...
`--dry-run` 检查平台能力、文件存在性/可读性/扩展名、目录起始序号、标题、定时及本机依赖。不会打开浏览器、登录、发布、转换图片或自动生成文案，也不会创建发布记录；标题来自显式输入、已有同名 JSON 或本地模板。它不验证平台登录状态或素材实际编码。图文转视频也会检查 MoviePy 和 ffmpeg；启用 B站时还会只读检查本地 biliup 程序。实际发布前同样检查全部素材并解析全部标题。
