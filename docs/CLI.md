...
首次运行会创建当前系统用户的 `~/.opub/` 数据目录。同一系统用户下的所有 Agent 固定使用这一目录；Windows、macOS 和 Linux 都由系统解析用户主目录，不需要设置额外环境变量。发布预检只检查环境，不安装或更新依赖。浏览器自动化需要 Chromium，图文转视频还需要 MoviePy/Pillow 和 ffmpeg。
...
记录固定存放在 `~/.opub/publish-history.sqlite3`，与执行目录和源码仓库无关。记录包含素材/账号文件路径、文案、标签、时间及发布结果，不保存 cookie 内容或激活凭据。账号路径固定，但不会核验同一路径下账号身份是否被替换，恢复前应保持原账号。删除记录会失去该任务的恢复保护。
...
浏览器平台按“每素材、每平台、每账号”管理会话：登录判定、必要扫码、上传和结果确认复用同一个浏览器上下文与页签，完成或异常后统一关闭。平台为提取作品链接而打开的临时页签仍沿用原有处理。不同素材、平台或账号之间不共享会话；命令结束后不保留窗口。B站继续使用 biliup 子进程及原有登录预检。仅明确发生在提交前的登录失效允许重新建立一次会话恢复，提交结果未知时仍禁止自动重试。
