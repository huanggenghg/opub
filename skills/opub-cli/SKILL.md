...
参数说明:`--title`/`--desc` 留空时尝试自动生成(需视频同名 JSON 或 ZHIPU_API_KEY),生成失败报 CFG-001,此时应补上 `--title` 重试;`--schedule` 指定后本次为定时发布;账号文件从数据目录 `cookies/` 自动发现(微博支持多账号,每个账号各发一遍),无需配置。**启用平台若无账号文件,发布时会自动弹出浏览器扫码登录**,登录完成后继续发布,不需要提前单独登录。
