...
| 错误码 | 含义 | 建议 |
| --- | --- | --- |
| `LIC001` | 未激活 | 选择微信或支付宝并执行对应激活命令 |
| `LIC002` | 激活失败 | 按付款窗口提示完成支付后重试 |
| `LIC003` | 付款未完成 | 完成付款后重新运行激活命令 |
| `LIC004` | 许可签名或数据无效 | 联系支持并提供错误码，不要提交内部日志 |
| `LIC010` | 设备已绑定其他许可 | 新电脑重新购买 |
| `LIC011` | 不支持迁移、解绑或换机重置 | 在原设备使用，或新电脑重新购买 |
| `LIC012` | 许可服务暂时不可用 | 稍后重试；首次激活前需恢复网络 |

退出码 `13`（`exit 13`）表示需要首次付费激活；显示 `LICxxx` 时按表中建议处理。
