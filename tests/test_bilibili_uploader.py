...
class ListBvsTests(unittest.TestCase):
    ...
    def test_returns_empty_set_for_empty_stdout(self):
        uploader = _make_uploader()
        with patch("uploader.bilibili_uploader.main.run_biliup_command_async", return_value=_make_completed(0, stdout="")):
            bvs = asyncio.run(uploader._list_bvs())
        self.assertEqual(bvs, set())

    def test_returns_empty_set_when_command_fails(self):
        uploader = _make_uploader()
        with patch("uploader.bilibili_uploader.main.run_biliup_command_async", return_value=_make_completed(1, stderr="cookie 失效")):
            bvs = asyncio.run(uploader._list_bvs())
        self.assertEqual(bvs, set())
...
class UploadWireTests(unittest.TestCase):
    def test_missing_video_does_not_start_any_command(self):
        ...
        with patch("os.path.exists", return_value=False), \
             patch("uploader.bilibili_uploader.main.run_biliup_command_async") as command:
    ...
    def test_upload_success_with_bv_sets_result_url(self):
        ...
        with patch("uploader.bilibili_uploader.main.run_biliup_command_async", side_effect=fake_run_biliup_command), \
    ...
    def test_upload_success_without_bv_leaves_result_url_absent(self):
        ...
        with patch("uploader.bilibili_uploader.main.run_biliup_command_async", return_value=upload_completed), \
             patch("os.path.exists", return_value=True), \
             patch.object(uploader, "_list_bvs", return_value=set()), \
    ...
    def test_upload_failure_does_not_call_capture(self):
        ...
        with patch("uploader.bilibili_uploader.main.run_biliup_command_async", return_value=upload_failed), \
             patch("os.path.exists", return_value=True), \
    ...
    def test_upload_timeout_is_unknown_and_not_safe_to_retry(self):
        ...
        with patch("os.path.exists", return_value=True), \
             ...
             patch("uploader.bilibili_uploader.main.run_biliup_command_async", side_effect=subprocess.TimeoutExpired(["biliup", "upload"], 3600)):
            result = asyncio.run(uploader.upload())
        self.assertFalse(result["success"])
    ...
    def test_successful_upload_stays_successful_when_link_query_raises(self):
        ...
        with patch("os.path.exists", return_value=True), \
             patch.object(uploader, "_list_bvs_with_status", return_value=(set(), True)), \
             patch("uploader.bilibili_uploader.main.run_biliup_command_async", return_value=_make_completed(0)), \
